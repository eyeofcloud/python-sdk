# Copyright 2016-2020, Eyeofcloud
...
from eyeofcloud.helpers import condition as condition_helper
...
class CustomAttributeConditionEvaluatorTest(base.BaseTest):
    ...
    def test_exact__given_number_values__calls_is_finite_number(self):
        ...
        with mock.patch('eyeofcloud.helpers.validator.is_finite_number', side_effect=[False, True]) as mock_is_finite:
            self.assertIsNone(evaluator.evaluate(0))
        ...
        with mock.patch('eyeofcloud.helpers.validator.is_finite_number', side_effect=[True, False]) as mock_is_finite:
            self.assertIsNone(evaluator.evaluate(0))
        ...
        with mock.patch('eyeofcloud.helpers.validator.is_finite_number', side_effect=[True, True]) as mock_is_finite:
    ...
    def test_greater_than__calls_is_finite_number(self):
        ...
        with mock.patch(
                'eyeofcloud.helpers.validator.is_finite_number',
        ...
        with mock.patch(
                'eyeofcloud.helpers.validator.is_finite_number', side_effect=is_finite_number__accepting_both_values,
        ):
            self.assertTrue(evaluator.evaluate(0))
    ...
    def test_evaluate__match_type__invalid(self):
        ...
        mock_log.assert_called_once_with(
            (
                ...
                'type. You may need to upgrade to a newer release of the Eyeofcloud SDK.'
    ...
    def test_evaluate__condition_type__invalid(self):
        ...
        mock_log.assert_called_once_with(
            (
                ...
                'You may need to upgrade to a newer release of the Eyeofcloud SDK.'
            ).format(json.dumps(expected_condition_log))
        )
    ...
    def test_exact__condition_value_invalid(self):
        ...
        mock_log.assert_called_once_with(
            (
                ...
                'newer release of the Eyeofcloud SDK.'
            ).format(json.dumps(expected_condition_log))
        )
