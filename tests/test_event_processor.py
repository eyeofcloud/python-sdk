# Copyright 2019-2020, Eyeofcloud
...
from eyeofcloud.event.payload import Decision, Visitor
from eyeofcloud.event.event_processor import (
    BatchEventProcessor,
    ForwardingEventProcessor,
)
from eyeofcloud.event.event_factory import EventFactory
from eyeofcloud.event.log_event import LogEvent
from eyeofcloud.event.user_event_factory import UserEventFactory
from eyeofcloud.event_dispatcher import EventDispatcher as default_event_dispatcher
from eyeofcloud.helpers import enums
from eyeofcloud.logger import NoOpLogger
from . import base
...
class BatchEventProcessorTest(base.BaseTest):
    ...
    def setUp(self, *args, **kwargs):
        ...
        self.eyeofcloud.logger = NoOpLogger()
        self.notification_center = self.eyeofcloud.notification_center
    ...
    def _set_event_processor(self, event_dispatcher, logger):
        self.event_processor = BatchEventProcessor(
            ...
            self.eyeofcloud.notification_center,
        )

    def test_drain_on_stop(self):
        event_dispatcher = CustomEventDispatcher()

        with mock.patch.object(self.eyeofcloud, 'logger') as mock_config_logging:
    ...
    def test_notification_center__on_log_event(self):
        ...
        self.eyeofcloud.notification_center.add_notification_listener(enums.NotificationTypes.LOG_EVENT, on_log_event)
        ...
        self.assertEqual(
            1, len(self.eyeofcloud.notification_center.notification_listeners[enums.NotificationTypes.LOG_EVENT]),
        )
...
class ForwardingEventProcessorTest(base.BaseTest):
    ...
    def test_event_processor__dispatch_raises_exception(self):
        ...
        user_event = self._build_conversion_event(self.event_name)
        log_event = EventFactory.create_log_event(user_event, self.eyeofcloud.logger)

        with mock.patch.object(self.eyeofcloud, 'logger') as mock_client_logging, mock.patch.object(
