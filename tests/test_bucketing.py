...
class BucketerTest(base.BaseTest):
    ...
    def test_bucket(self):
        ...
        # Variation 1
        with mock.patch(
            'eyeofcloud.bucketer.Bucketer._generate_bucket_value', return_value=42
        ...
        # Empty entity ID
        with mock.patch(
            'eyeofcloud.bucketer.Bucketer._generate_bucket_value', return_value=4242
        ) as mock_generate_bucket_value:
            variation, _ = self.bucketer.bucket(
                self.project_config,
                self.project_config.get_experiment_from_key('test_experiment'),
        ...
        # Variation 2
        with mock.patch(
            'eyeofcloud.bucketer.Bucketer._generate_bucket_value', return_value=5042
        ) as mock_generate_bucket_value:
            variation, _ = self.bucketer.bucket(
                self.project_config,
                self.project_config.get_experiment_from_key('test_experiment'),
        ...
        with mock.patch(
            'eyeofcloud.bucketer.Bucketer._generate_bucket_value', return_value=424242
        ) as mock_generate_bucket_value:
            variation, _ = self.bucketer.bucket(
                self.project_config,
                self.project_config.get_experiment_from_key('test_experiment'),
    ...
    def test_bucket__experiment_in_group(self):
        ...
        with mock.patch(
            'eyeofcloud.bucketer.Bucketer._generate_bucket_value', side_effect=[42, 4242],
        ) as mock_generate_bucket_value:
            variation, _ = self.bucketer.bucket(
                self.project_config,
        ...
        with mock.patch(
            'eyeofcloud.bucketer.Bucketer._generate_bucket_value', side_effect=[42, 9500],
        ) as mock_generate_bucket_value:
            variation, _ = self.bucketer.bucket(
                self.project_config,
                self.project_config.get_experiment_from_key('group_exp_1'),
        ...
        with mock.patch(
            'eyeofcloud.bucketer.Bucketer._generate_bucket_value', side_effect=[42, 424242],
        ) as mock_generate_bucket_value:
            variation, _ = self.bucketer.bucket(
                self.project_config,
                self.project_config.get_experiment_from_key('group_exp_1'),
...
class BucketerWithLoggingTest(base.BaseTest):
    ...
    def test_bucket(self):
        ...
        # Variation 1
        with mock.patch('eyeofcloud.bucketer.Bucketer._generate_bucket_value', return_value=42), mock.patch.object(
        ...
        # Empty entity ID
        with mock.patch('eyeofcloud.bucketer.Bucketer._generate_bucket_value', return_value=4242), mock.patch.object(
            self.project_config, 'logger'
        ) as mock_config_logging:
            variation, _ = self.bucketer.bucket(
                self.project_config,
        ...
        # Variation 2
        with mock.patch('eyeofcloud.bucketer.Bucketer._generate_bucket_value', return_value=5042), mock.patch.object(
            self.project_config, 'logger'
        ) as mock_config_logging:
            variation, _ = self.bucketer.bucket(
                self.project_config,
        ...
        # No matching variation
        with mock.patch('eyeofcloud.bucketer.Bucketer._generate_bucket_value', return_value=424242), mock.patch.object(
            self.project_config, 'logger'
        ) as mock_config_logging:
            variation, _ = self.bucketer.bucket(
                self.project_config,
    ...
    def test_bucket__experiment_in_group(self):
        ...
        with mock.patch(
            'eyeofcloud.bucketer.Bucketer._generate_bucket_value', side_effect=[8400, 9500],
        ), mock.patch.object(self.project_config, 'logger') as mock_config_logging:
            variation, _ = self.bucketer.bucket(
                self.project_config,
                self.project_config.get_experiment_from_key('group_exp_1'),
